...
Blockly.Procedures.flyoutCategory = function(workspace) {
  ...
  for (var i = 0; i < mutations.length; i++) {
    ...
    let return_data = block.getElementsByTagName("mutation")[0].getAttribute("return")
    console.log("res:"+return_data)
    if (return_data == "false" || return_data == "null" ){
      block.setAttribute('type', 'procedures_call');
    }else{
      block.setAttribute('type', 'procedures_call_with_return');
    }
    console.log("new:"+block.getElementsByTagName("mutation")[0].getAttribute("return"))
    //console.log(block.getElementsByTagName("mutation")[0].getAttribute("return"))//.getAttribute("return")
    xmlList.push(block);
  }
  return xmlList;
};
...
Blockly.Procedures.getCallers = function(name, ws, definitionRoot,
  ...
  for (var i = 0; i < allBlocks.length; i++) {
    ...
    if (block.type == Blockly.PROCEDURES_CALL_BLOCK_TYPE || block.type == Blockly.PROCEDURES_RETURN_CALL_BLOCK_TYPE) {
...
Blockly.Procedures.editProcedureCallback_ = function(block) {
  ...
  if (block.type == Blockly.PROCEDURES_DEFINITION_BLOCK_TYPE) {
    ...
    block = innerBlock;
  } else if (block.type == Blockly.PROCEDURES_RETURN_CALL_BLOCK_TYPE || block.type == Blockly.PROCEDURES_CALL_BLOCK_TYPE) {
